    ...
    <template v-else>
      ...
      <div v-else class="flex flex-col items-start w-full lg:gap-8 mb-24 lg:flex-row">
        <nav class="flex lg:grid flex-wrap w-full gap-1 my-8 text-gray-600 min-w-[240px] top-24 lg:w-auto lg:sticky">
          <NuxtLink to="/my-account?tab=my-details" class="flex items-center gap-4 py-4 rounded-lg hover:text-primary" :class="{ active: activeTab == 'my-details' }">
            <Icon name="ion:information-circle-outline" size="22" />
            {{ $t('messages.general.myDetails') }}
          </NuxtLink>
          <NuxtLink to="/my-account?tab=orders" class="flex items-center gap-4 py-4 rounded-lg hover:text-primary" :class="{ active: activeTab == 'orders' }">
            <Icon name="ion:bag-check-outline" size="22" />
            {{ $t('messages.shop.order', 2) }}
          </NuxtLink>
          <NuxtLink to="/my-account?tab=downloads" class="flex items-center gap-4 py-4 rounded-lg hover:text-primary" :class="{ active: activeTab == 'downloads' }">
            <Icon name="ion:cloud-download-outline" size="22" />
            {{ $t('messages.general.downloads') }}
          </NuxtLink>
          <button class="flex items-center gap-4 py-4 rounded-lg hover:text-primary" @click="logoutUser">
